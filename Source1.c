...
int main(void) {
    ...
    if (strcmp(mode, "Solve") == 0 || strcmp(mode, "solve") == 0) {
        ...
        for (int i = 0;i < nom;i++) {
            struct mass lala;
            lala.m = 1;
            lala.x = i + 1;                                     //Initial conditions
            lala.y = 0;
            lala.dx = 0;
            lala.dy = 0;
            masses_array[i] = lala;
        }
        double* results = malloc(steps * nom * 2 * sizeof(double));if (!results) { return 1; }             // [x,y]
        ...
        double* J = (double*)calloc(n_phi * n * nom * 2, sizeof(double)); if (!J) { return 1; }            // Jacobian
